...
#define EXPECT_ERROR(expression)                         \
  {                                                      \
    try {                                                \
      expression;                                        \
      assert(0 && "expected exception in " #expression); \
  ...
  if (which == "printf") {
    ...
    for (long i = 0; i < maxIter; ++i)
      std::cout << std::setprecision(10) << std::fixed << 1.234 << ':'
                << std::resetiosflags(std::ios::floatfield) << std::setw(4)
                << std::setfill('0') << 42 << std::setfill(' ') << ':'
                << std::setiosflags(std::ios::showpos) << 3.13
...
#  ifdef HAVE_FORMAT
    ...
    for (long i = 0; i < maxIter; ++i) {
      ...
      auto finished_at = fmt::format_to(buf, FMT_COMPILE("{:.10f}"), 1.234);
      *finished_at = '\0';
      std::puts(buf);
    }
  }
